...
{
    path_abs = ros::package::getPath("eddie_bringup") + path_local;
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
    ROS_INFO("Initiated gps_navigation node");
    ...
    while (!ac.waitForServer(ros::Duration(5.0)))
    {
        ...
        if (wait_count > 3)
        {
            ROS_ERROR("move_base action server did not come up, killing gps_navigation node...");
    ...
    // Get Longitude and Latitude goals from text file and count number of waypoints
    ros::param::get("/outdoor_waypoint_nav/coordinates_file", path_local);

    path_local = "/points/points.txt";
